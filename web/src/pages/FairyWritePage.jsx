...
export default function FairyWritePage() {
  const navigate = useNavigate();
  const location = useLocation();
  const bodyRef = useRef(null);
  const selectedKeywords = Array.isArray(location.state?.keywords)
    ? location.state.keywords
    : [];

  const DRAFT_STORAGE_KEY = "fairyWritingDrafts_v1";
  const LAST_DRAFT_KEY = "fairyWritingDraft";
  const mainKeyword = selectedKeywords[0] || "왕국";
  ...
  const topicParticle = hasBatchim(mainKeyword) ? "을" : "를";
  ...
  const previewTags = useMemo(
    () => selectedKeywords.map((keyword) => `키워드: ${keyword}`),
    [selectedKeywords]
  );
  ...
  const saveDraftsToStorage = (nextDrafts) => {
    try {
      sessionStorage.setItem(DRAFT_STORAGE_KEY, JSON.stringify(nextDrafts));
    } catch {
      // ignore
    }
  };
  ...
  const handleTempSave = () => {
    ...
    try {
      // 마지막 임시 저장본 (이전 호환용)
      sessionStorage.setItem(LAST_DRAFT_KEY, JSON.stringify(draft));
    ...
    <div className="write-page">
      ...
      <section className="write-header">
        <div className="write-header-text">
          ...
          <h2 className="write-title">오늘의 동화를 써봐요</h2>
          <p className="write-subtitle">
            {mainKeyword}
            {topicParticle} 주제로 한 동화를 써봐요.
          </p>
          {selectedKeywords.length > 0 && (
            <div className="preview-tags" style={{ marginTop: 10 }}>
              {selectedKeywords.map((keyword) => (
                <span className="tag-pill" key={keyword}>
                  {keyword}
                </span>
              ))}
            </div>
